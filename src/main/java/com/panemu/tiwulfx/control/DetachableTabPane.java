...
public class DetachableTabPane extends TabPane {
	...
	private void initiateDragGesture() {
		Set<Node> tabUI = this.lookupAll(".detachable-tab-pane > .tab-header-area .tab");
		if (tabUI.isEmpty() && !getTabs().isEmpty()) {
			Platform.runLater(() -> initiateDragGesture());
		}
		for (Node node : tabUI) {
	...
	private void calculateTabPoints() {
		lstTabPoint.clear();
		lstTabPoint.add(0d);
		Set<Node> tabs = this.lookupAll(".detachable-tab-pane > .tab-header-area .tab");
